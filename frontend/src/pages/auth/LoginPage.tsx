...
export const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginInput) => {
    ...
    try {
      ...
      const next = location.state?.from || home[(res as any)?.data?.role as keyof typeof home] || '/';
          ...
          <div className="max-w-md w-full">
            ...
            <div className="mb-10">
              <h1 className="text-4xl lg:text-4xl font-heading font-black italic tracking-tighter uppercase leading-none mb-2 text-left">
                Welcome Back <br />
                <span className="not-italic font-sans text-sm tracking-[0.5em] font-bold opacity-40 text-left">
                  Login to Continue
                </span>
              </h1>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="space-y-2 text-left">
                <div className="flex justify-between items-center px-1">
                  ...
                  <Link to="/forgot-password" className="text-[9px] text-[#1A1A1A]/40 font-medium uppercase tracking-widest hover:text-[#1A1A1A]">
                    Forgot Password
                  </Link>
                </div>
                <div className="relative group">
              ...
              {/* ── Backend error shown here, after the last field ── */}
              {serverError && (
              ...
              <div className="flex items-center gap-4 py-4 px-1">
                <input
                  type="checkbox"
                  id="remember"
                  className="w-4 h-4 border-[#1A1A1A]/10 rounded-none text-[#1A1A1A] focus:ring-0"
                />
                <label htmlFor="remember" className="text-[10px] font-bold uppercase tracking-widest opacity-40 cursor-pointer">
                  Remember Me
                </label>
              </div>
              ...
              <Button
                type="submit"
                className="w-full h-14 text-[10px] font-bold uppercase tracking-[0.4em] bg-[#1A1A1A] text-[#FDFCF8] hover:bg-black transition-all"
              >
